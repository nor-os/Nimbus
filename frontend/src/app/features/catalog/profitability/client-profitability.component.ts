/**
 * Overview: Client profitability breakdown — table showing per-client financial metrics.
 * Architecture: Catalog feature component (Section 8)
 * Dependencies: @angular/core
 * Concepts: Client profitability, margin analysis
 */
import {
  Component,
  Input,
...
@Component({
  selector: 'nimbus-client-profitability',
  standalone: true,
      ...
      <table class="table">
        <thead>
          <tr>
            <th class="sortable" (click)="onSort('entityName')">
              Client Name <span class="sort-icon">{{ getSortIcon('entityName') }}</span>
            </th>
        ...
        <tbody>
          @for (row of sortedData(); track row.entityId) {
            ...
            <tr>
              <td colspan="6" class="empty-state">No client profitability data available</td>
            </tr>
          }
        </tbody>
      </table>
