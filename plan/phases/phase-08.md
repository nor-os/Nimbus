...
*Was Phase 5. Now Phase 8. Dependency inverted: now depends on Semantic Layer (Phase 5) instead of the other way around.*

## Deliverables
- Comprehensive CI classes based on semantic types (Infrastructure, Services, Security, Monitoring, Cost centers)
...
## Dependencies
- Phase 5 complete (semantic types as basis for CI classes)
...
**Rationale**: Full infrastructure coverage plus service catalog for MSP/service provider business model. CI classes map to semantic types from Phase 5.
...
**Rationale**: Keep Phase 8 focused; bulk import can be added later if needed.

### Q13: Class Management
**Question**: How to manage CI class definitions?
**Decision**: Extend system
**Rationale**: System classes (from semantic types) provide foundation, tenants can add custom classes.
...
**Rationale**: Phase 2 creates basic Compartment model (id, tenant_id, parent_id, name, description). Phase 8 extends it with CMDB-specific fields (cloud_id, provider_type) and adds the full hierarchy service.
...
```
CI Classes (based on Phase 5 Semantic Types)
├── Infrastructure
│   ├── Compute
│   │   ├── VirtualMachine
│   │   ├── Container
...
#### Task 8.1: CI Core Data Models
**Complexity**: L
**Description**: Create core CMDB models for CIs and classes. Extend Phase 2 Compartment model with CMDB fields. CI classes derive from Phase 5 semantic types.
**Files**:
...
- `backend/alembic/versions/XXX_cmdb_core.py` - Migration (extends Compartment table)
**Acceptance Criteria**:
...
- [ ] CIClass: id, tenant_id (null for system), name, parent_class_id, semantic_type_id, schema (JSON), icon, is_system, is_active
...
**Note**: Compartment base model (id, tenant_id, parent_id, name, description) created in Phase 2. This task adds CMDB-specific extensions. CIClass links to Phase 5 semantic types via semantic_type_id.
**Tests**:
- [ ] CI CRUD works
...
#### Task 8.2: Graph Relationship Models
**Complexity**: L
...
#### Task 8.3: CI Snapshot Versioning
**Complexity**: M
...
#### Task 8.4: Service Catalog Models
**Complexity**: M
...
#### Task 8.6: Validation Rules Engine
**Complexity**: M
...
#### Task 8.9: Advanced Search Service
**Complexity**: L
...
#### Task 8.10: CI Class Management Service
**Complexity**: M
**Description**: Manage CI class definitions. System classes seed from Phase 5 semantic types.
**Files**:
- `backend/app/services/cmdb/class_service.py` - CI class management
- `backend/app/services/cmdb/class_seed.py` - System class seeding from semantic types
**Acceptance Criteria**:
- [ ] Seed system CI classes from Phase 5 semantic types on initialization
...
**Tests**:
- [ ] System classes seeded from semantic types
...
#### Task 8.11: CMDB GraphQL API
**Complexity**: L
**Description**: GraphQL schema for CMDB (REST for auth only, GraphQL for data).
**Files**:
...
#### Task 8.12: Service Catalog GraphQL API
**Complexity**: M
**Description**: GraphQL endpoints for service catalog.
**Files**:
- `backend/app/api/graphql/types/catalog.py` - Catalog types
- `backend/app/api/graphql/queries/catalog.py` - Catalog queries
- `backend/app/api/graphql/mutations/catalog.py` - Catalog mutations
**Acceptance Criteria**:
- [ ] ServiceOfferings: query + mutations
- [ ] PriceLists: query + mutations
- [ ] PriceListItems: query + mutations
- [ ] TenantPriceOverrides: query + mutations
- [ ] effectivePrice(tenantId, serviceId) query
- [ ] serviceOfferings(category) filtered query
...
#### Task 8.13: CMDB Service (Frontend)
**Complexity**: M
...
#### Task 8.17: Compartment Tree View
**Complexity**: M
...
#### Task 8.18: Relationship Graph Visualization
**Complexity**: L
...
#### Task 8.19: Impact Analysis View
**Complexity**: M
...
#### Task 8.20: CI Template Management
**Complexity**: M
...
- Phase 11 (Proxmox Provider) will build on CI classes for resource storage
- Phase 13 (Real-time) will add live CMDB updates
